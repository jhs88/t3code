...
const piRuntimeDriver = ProviderDriverKind.make("pi");
const piRuntimeInstanceId = ProviderInstanceId.make("pi-runtime-test");
const piRuntimeAdapter = makeFakeCodexAdapter(piRuntimeDriver, false);
const unrestrictedAdapter = makeFakeCodexAdapter(CODEX_DRIVER);
const piRuntimeRouting = makeProviderServiceLayer({
  registry: makeStaticInstanceRegistry([
    [
      piRuntimeInstanceId,
      {
        ...
        capabilities: {
          ...piRuntimeAdapter.adapter.capabilities,
          allowedRuntimeModes: ["full-access"],
          runtimeModeReason: "Pi requires full access.",
        },
      },
    ],
    [codexInstanceId, unrestrictedAdapter.adapter],
  ]),
});

piRuntimeRouting.layer("ProviderService Pi runtime restrictions", (it) => {
  it.effect(
    "rejects restricted starts and provider switches without restricting upstream auto mode",
    () =>
      Effect.gen(function* () {
        const provider = yield* ProviderService.ProviderService;
        const threadId = asThreadId("pi-runtime-restrictions");
        const input = {
          threadId,
          providerInstanceId: piRuntimeInstanceId,
          runtimeMode: "approval-required" as const,
        };
        const failure = yield* provider.startSession(threadId, input).pipe(Effect.flip);
        assert.instanceOf(failure, ProviderValidationError);
        assert.include(failure.issue, "Pi requires full access");
        assert.equal(piRuntimeAdapter.startSession.mock.calls.length, 0);
        ...
        const switched = yield* provider.startSession(threadId, input).pipe(Effect.flip);
        assert.instanceOf(switched, ProviderValidationError);
        assert.equal(piRuntimeAdapter.startSession.mock.calls.length, 0);

        yield* provider.startSession(threadId, { ...input, runtimeMode: "full-access" });
        const rollback = yield* provider
          .rollbackConversation({ threadId, numTurns: 1 })
          .pipe(Effect.flip);
        assert.instanceOf(rollback, ProviderValidationError);
        assert.equal(piRuntimeAdapter.rollbackThread.mock.calls.length, 0);
