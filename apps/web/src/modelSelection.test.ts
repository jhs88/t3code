...
describe("instance-scoped model selection", () => {
  it("uses Pi's discovered models without requiring legacy custom-model settings", () => {
    const providers = [
      provider({
        provider: ProviderDriverKind.make("pi"),
        instanceId: "pi",
        models: ["test/pi-model"],
      }),
    ];
    ...
    ).toEqual(["test/pi-model"]);
    expect(
      getCustomModelOptionsByInstance(DEFAULT_UNIFIED_SETTINGS, providers)
        .get(ProviderInstanceId.make("pi"))
