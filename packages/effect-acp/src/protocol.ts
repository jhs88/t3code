...
export const makeAcpPatchedProtocol = Effect.fn("makeAcpPatchedProtocol")(function* (
  ...
  const handleExitEncoded = (incoming: RpcMessage.ResponseExitEncoded) => {
    // Effect's JSON-RPC decoder treats ordinary JSON-RPC errors as defects.
    // ACP peers do not send Effect's private Cause envelope; recover their
    // validated protocol errors before the typed RPC client decodes the exit.
    const message: RpcMessage.ResponseExitEncoded =
      incoming.exit._tag === "Failure"
        ? {
            ...incoming,
            exit: {
              ...incoming.exit,
              cause: incoming.exit.cause.map((entry) =>
