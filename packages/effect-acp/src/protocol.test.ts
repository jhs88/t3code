...
it.layer(NodeServices.layer)("effect-acp protocol", (it) => {
  ...
  it.effect("turns a plain ACP error into a typed failure for the standard client", () =>
    Effect.gen(function* () {
      const { stdio, input } = yield* makeInMemoryStdio();
      const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
        stdio,
      ...
      yield* Queue.offer(
        input,
        encoder.encode(
          `${encodeUnknownJsonString({ jsonrpc: "2.0", id: 42, error: { code: -32002, message: "Session not found" } })}\n`,
        ),
      );
      assert.deepEqual(yield* Deferred.await(received), {
        ...
        exit: {
          _tag: "Failure",
          cause: [{ _tag: "Fail", error: { code: -32002, message: "Session not found" } }],
        },
      });
    }),
  );
